...
resource "aws_apigatewayv2_domain_name" "shout_domain_name" {
  # api gateway에 커스텀 도메인 등록
...
resource "aws_route53_record" "api_gateway" {
  ...
  name    = aws_apigatewayv2_domain_name.shout_domain_name.domain_name
  type    = "A"
  zone_id = aws_route53_zone.zone_main.zone_id

  alias {
    name                   = aws_apigatewayv2_domain_name.shout_domain_name.domain_name_configuration[0].target_domain_name
    zone_id                = aws_apigatewayv2_domain_name.shout_domain_name.domain_name_configuration[0].hosted_zone_id
...
resource "aws_apigatewayv2_api_mapping" "api_gateway" {
  ...
  domain_name = aws_apigatewayv2_domain_name.shout_domain_name.id
